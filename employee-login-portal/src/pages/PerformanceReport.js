...
import { FiFilter, FiDownload, FiSearch, FiUser, FiTarget, FiTag, FiTrendingUp } from "react-icons/fi";
...
const PerformanceReport = () => {
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            const res = await api.get(`/goals/fetch-reports?${params.toString()}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDownloadExport = () => {
        ...
        const dataToExport = reportsData.map(item => ({
            ...
            "Weightage": item.weightage || "N/A",
            "Start Date": item.startDate,
            "End Date": item.endDate,
            "Status": item.status,
            "Rating": item.rating || "N/A",
            "Manager Rating": item.managerRating || "N/A"
        }));
