...
const HelpDeskCreateNewTicket = () => {
    ...
    // State for file management (simplified)
    const [selectedFile, setSelectedFile] = useState(null);

    // State to manage button hover for styling
    ...
    const loadCategories = async () => {
        try {
            ...
            const res = await api.get(
                `/v1/helpdesk-categories/ticket-type?ticketType=NEW_TICKET`,
            ...
            if (resubmitTicket) {
                ...
                if (foundCat) {
                    setFormData(prev => ({
                        ...
                        ccToManager: resubmitTicket.ccToManager || false
                    }));
            ...
            if (editDraft) {
                ...
                if (foundCat) {
                    setFormData(prev => ({
                        ...
                        ccToManager: editDraft.ccToManager || false
                    }));
    ...
    const handleChange = (e) => {
        ...
        setFormData(prevData => ({
            ...prevData,
            [name]: (name === 'category' ? value : (type === 'checkbox' ? checked : value)),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.status === 200 || res.status === 201) {
                alert(isResubmitting ? "Ticket resubmitted successfully!" : "Ticket submitted successfully! Ticket ID: " + res.data.id);

                // Delete the draft if we were editing one
                if (isEditingDraft && editDraft) {
                    try {
                        const token = sessionStorage.getItem("token");
                        await api.delete(`/tickets/draft/${editDraft.id}`, {
                            headers: { Authorization: `Bearer ${token}` }
                        });
                    } catch (deleteErr) {
            ...
            const errorMessage = err.response?.data?.message || err.response?.data || err.message || "Error submitting ticket. Try again.";
            // If the error data is an object or large HTML, alert might be messy. Try to keep it clean.
    ...
    const handleSaveAsDraft = async () => {
        ...
        try {
            ...
            const res = await api.post("/tickets/draft", fd, {
                headers: {
                    "Content-Type": "multipart/form-data",
                    Authorization: `Bearer ${token}`,
                },
    ...
    // Determine the final style for the button
        ...
        <>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
                <div style={{
                    ...
                    <span>
                        Please note that all tickets may take up to <strong>5 working days</strong> to resolve. We request your patience and cooperation.
                    </span>
                </div>
                ...
                {/* CC to Manager Circular Checkbox */}
                <div className="grievance-form-group full-width" style={{ marginTop: '1rem', marginBottom: '1rem' }}>
                    <label className="grievance-checkbox-label">
                      <input
                        type="checkbox"
                        checked={formData.ccToManager}
                        onChange={(e) => handleChange({ target: { name: 'ccToManager', value: e.target.checked, type: 'checkbox', checked: e.target.checked } })}
                        className="grievance-checkbox-hidden"
                      />
                      <span className="grievance-custom-checkbox"></span>
                      <span className="grievance-checkbox-text">CC to Manager (Optional)</span>
                    </label>
                </div>
...
export default HelpDeskCreateNewTicket;
