...
function ManagerDashBoard({ embedded, employeeId: propEmployeeId, refreshTrigger, onActionComplete, statusFilter = 'All', onDataLoaded }) {
    ...
    if (effectiveId) {
      ...
      const fetchProfile = async () => {
        try {
          ...
          const res = await api.get(`/profile/${effectiveId}`, {
            headers: {
  ...
  useEffect(() => {
    ...
    if (statusFilter && statusFilter !== 'All') {
      ...
      filtered = filtered.filter(claim =>
        normalize(claim.status) === normalize(statusFilter)
      );
    }
    ...
    if (searchTerm.length > 0) {
      ...
      filtered = filtered.filter(claim => {
        return (
          String(claim.id).includes(lowerCaseSearchTerm) ||
          String(claim.employeeId).includes(lowerCaseSearchTerm) ||
          (claim.name && claim.name.toLowerCase().includes(lowerCaseSearchTerm))
        );
      });
    }
  ...
  const handleApprove = (id) => {
    ...
    api
      .post(`/claims/approve/${id}?role=Manager`, null, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then(() => {
        ...
        setOriginalClaims(prev => prev.filter(claim => claim.id !== id));
  ...
  const handleRejectClick = async (claimId) => {
    ...
    try {
      setLoading(true);
      await api.post(
        `/claims/reject/${claimId}?role=Manager&reason=${encodeURIComponent(reason)}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleApproveBulk = async (groupClaims) => {
    ...
    try {
      ...
      for (const claim of groupClaims) {
        await api.post(`/claims/approve/${claim.id}?role=Manager`, null, {
  ...
  const handleRejectBulk = async (groupClaims) => {
    ...
    try {
      setLoading(true);
      for (const claim of groupClaims) {
        await api.post(`/claims/reject/${claim.id}?reason=${encodeURIComponent(reason)}&role=Manager`, null, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
  ...
  const handleIndividualApprove = async (claimId) => {
    const token = sessionStorage.getItem("token");
    const confirmed = window.confirm("Are you sure you want to approve this claim?");
    if (!confirmed) return;

    try {
      setLoading(true);
      await api.post(`/claims/approve/${claimId}?role=Manager`, null, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert("Claim approved successfully");

      // Update local state to reflect change without closing bulk modal if possible
      // But fetchClaims is simpler to maintain consistency
      fetchClaims(employeeId);
      if (onActionComplete) onActionComplete();
    } catch (error) {
  ...
  const handleIndividualReject = async (claimId) => {
    const token = sessionStorage.getItem("token");
    const reason = window.prompt("Enter rejection reason:");
    if (reason === null) return;
    if (!reason.trim()) {
    ...
    try {
      setLoading(true);
      await api.post(`/claims/reject/${claimId}?role=Manager&reason=${encodeURIComponent(reason)}`, null, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert("Claim rejected successfully");
                  ...
                  <div className="modal-body-v2" style={{ maxHeight: '75vh', overflowY: 'auto', padding: '1.25rem' }}>
                    ...
                    <div className="bulk-table-container">
                      <table className="status-table">
                        <thead>
                          <tr>
                            <th style={{ paddingLeft: '6px' }}>Category</th>
                              ...
                              <td>
                                <div className="action-btn-group">
                                  <button
                                    ...
                                    onClick={() => handleIndividualApprove(claim.id)}
                                    title="Approve Claim"
                                  >
                                    <FiCheck size={16} />
                                  </button>
                                  <button
                                    ...
                                    onClick={() => handleIndividualReject(claim.id)}
                                    title="Reject Claim"
                                  >
                                    <FiX size={16} />
                                  </button>
                  ...
                  <div className="item-content" style={{ width: '100%' }}>
                    ...
                    <div className="action-buttons" style={{ display: "flex", gap: "12px", marginTop: "8px" }}>
                      <button
                        ...
                        style={{ flex: 1, padding: "8px", background: "linear-gradient(135deg, #5eead4 0%, #2dd4bf 100%)", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "600", fontSize: "0.85rem" }}
                      >
                        Approve Claim
                      </button>
                      <button
                        ...
                        style={{ flex: 1, padding: "8px", backgroundColor: "#f87171", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "600", fontSize: "0.85rem" }}
                      >
                        Reject Claim
                      </button>
                    </div>
