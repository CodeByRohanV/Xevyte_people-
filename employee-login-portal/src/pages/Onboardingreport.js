...
import { FiFilter, FiDownload, FiSearch, FiUsers, FiMapPin, FiBriefcase } from "react-icons/fi";
...
const Onboardingreport = () => {
    ...
    const handleSuggestionClick = (employee) => {
        ...
        if (currentIds.length > 0) {
            currentIds[currentIds.length - 1] = `${employee.firstName} ${employee.lastName} (${employee.employeeId})`;
        } else {
            currentIds.push(`${employee.firstName} ${employee.lastName} (${employee.employeeId})`);
        }
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            console.error("Failed to fetch onboarding report", err);
    ...
    const handleDownload = () => {
        ...
        const dataToExport = reportsData.map(emp => ({
            ...
            "Joining Date": emp.joiningDate ? formatDateToIST(emp.joiningDate) : "N/A",
            "Work Location": emp.workLocation || "N/A",
            "Department": emp.department || "N/A",
            "Employee Type": emp.employeeType || "N/A",
            "Email": emp.email,
            "Contact": emp.contactNo || "N/A",
            "Active Status": emp.active === 'yes' ? 'Active' : 'Inactive'
        }));

        const ws = XLSX.utils.json_to_sheet(dataToExport);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Onboarding Report");
        XLSX.writeFile(wb, `Onboarding_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
                            ...
                            {reportsData.length > 0 ? (
                                reportsData.map(emp => (
                                    <tr key={emp.employeeId}>
                                        ...
                                        <td>{emp.joiningDate ? formatDateToIST(emp.joiningDate) : "N/A"}</td>
                                        <td>{emp.role}</td>
                                        <td>{emp.gender || "N/A"}</td>
                                        <td>{emp.workLocation || "N/A"}</td>
                                        <td>
                                            <span className={`status-pill ${emp.active === 'yes' ? 'active' : 'inactive'}`}>
